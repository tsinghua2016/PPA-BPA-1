  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
        ...
        if (TaskState.isFinished(state)) {
          executorDataMap.get(executorId) match {
            case Some(executorInfo) =>
             logInfo("TaskState.isFinished")
              if(Prediction.isCustomize) {
                executorInfo.freeCores += Prediction.getTaskCpuCore(taskId)
                Prediction.removeTaskId(taskId)
                //makeOffers()
              }
              else
                { 
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      case RegisterExecutor(executorId, executorRef, hostPort, cores, logUrls) =>
        ...
        if (executorDataMap.contains(executorId)) {
          ...
          //prediction  make freecore=100
          if(Prediction.isCustomize)
            {
              data.freeCores=100;
             // logWarning("prediction => customize make freeCore "+data.freeCores)
    ...
    private def makeOffers() {
      ...
      logInfo("makeOffers() " +workOffers.size)
      launchTasks(scheduler.resourceOffers(workOffers))
      //logWarning("launch tasks")
    ...
    private def makeOffers(executorId: String) {
      // Filter out executors under killing
      //logWarning("makeOffers(executorId: String) ")
      if (!executorsPendingToRemove.contains(executorId)) {
        val executorData = executorDataMap(executorId)
        val workOffers = Seq(
          new WorkerOffer(executorId, executorData.executorHost, executorData.freeCores))
        logInfo("makeOffers(executorId: String) "+workOffers.size)
        launchTasks(scheduler.resourceOffers(workOffers))
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= akkaFrameSize - AkkaUtils.reservedSizeBytes) {
          ...
          //logWarning("active "+task.taskId)
          val executorData = executorDataMap(task.executorId)
          if(Prediction.isCustomize) {
            executorData.freeCores -= Prediction.getTaskCpuCore(task.taskId)
            logInfo("247:executorData.freeCores => "+executorData.freeCores+" fit=> "+Prediction.getTaskCpuCore(task.taskId))
          }
          else
